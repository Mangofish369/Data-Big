...
def test_model(kyc_people_df: pd.DataFrame, kyc_business_df: pd.DataFrame, transaction_dfs: dict[str, pd.DataFrame]
...
# Drop na and set name of subset
...
def train(full_df: pd.DataFrame, labeled_df: pd.DataFrame, drop_cols: list[str], lr_seed=0, split_seed=24,
    ) -> list[dict]:
    ...
    final_predictions = (probabilities >= HIGH_RISK_THRESHOLD).astype(int)
